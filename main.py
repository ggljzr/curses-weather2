...
def refresh_weather(current_weather_data, forecast_weather_data, location):
    ...
    fore_win.draw_forecast(forecast_data, 0)
...
if __name__ == '__main__':
    ...
    current_data, location = api.request_current_data(config['weather']['locations'][0])
    forecast_data = api.request_forecast_data(config['weather']['locations'][0])
    ...
    while True:
        ...
        if c == ord('q'):
            break
        if c == curses.KEY_F5:
            current_data, location = api.request_current_data(config['weather']['locations'][current_loc])
            forecast_data = api.request_forecast_data(config['weather']['locations'][current_loc])
            refresh_weather(current_data, forecast_data, location)
