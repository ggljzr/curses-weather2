...
class Location_window(cg.Window):
    ...
    def draw_location(self, location):
        ...
        try:
            self.window.addstr(1, 1, location.city, curses.A_BOLD)
            self.window.addstr(2, 1, str_lon)
            self.window.addstr(3, 1, str_lat)
            self.window.addstr(4, 1, str_country)
        except curses.error:
            pass
...
class Forecast_window(cg.Window):
    ...
    def drawDay(self, yPos, xPos, dayData):
        ...
        try:
            ...
            self.window.addstr(yPos + 1, xPos, time,
                                  curses.A_BOLD | curses.color_pair(cg.PAIR_YELLOW))
            icons.drawIcon(xPos, yPos + 2, icon, self.window)
            self.window.addstr(yPos + icons.iconHeight + 2, xPos, temp,
                                  curses.color_pair(cg.PAIR_GREEN))
            self.window.addstr(yPos + icons.iconHeight + 3, xPos, humidity,
                                  curses.color_pair(cg.PAIR_GREEN))
            self.window.addstr(yPos + icons.iconHeight + 4, xPos, pressure,
                                  curses.color_pair(cg.PAIR_GREEN))
        except curses.error:
            pass

    def draw_forecast(self, forecast_data, hour):
